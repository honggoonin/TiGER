...
// (3) Send c1h_a and c1h_b from mod q to mod k1(64) and mod k2(16).
	...
	for (i=0; i< LWE_N; ++i) {
		c_t[i] = ((c1[i] + 0x02) & 0xfc); // modified c -> c_t
		c_t[LWE_N + i] = ((c2[i] + 0x08) & 0xf0); // modified c -> c_t    // 4=0x20/0xc0 8=0x10/0xe0, 16=0x08/0xf0, 32=0x04/0xf8 64=0x02/0xfc, 128= 0x01 0xfe
	}
///////////////////////////////////////////////////////////////////////////////////////////////
...
///////////////////////////////////////////////////////////////////////////////////////////////
/////
	memcpy(c1_hat, c_t, LWE_N); // modified c-> c_t
	memcpy(decomp_delta, c_t+LWE_N, LWE_N); // modified c-> c_t
...
*/
// (2)은 sk배열이 이미 -1, 1값의 인덱스를 저장한 배열로 바뀌었기 때문에 필요없음. 다만 계산을 위해 neg_start가 필요한데 그것은 sk배열의 맨 마지막(sk[161])에 저장해 뒀음.//
	int neg_start = sk[161];
//// Step2 : Compute Message_hat_prime = c2 - c1 * s. ////

	for(i = 0; i < HS; ++i){
		uint16_t branch = (2 * ((i - neg_start) >> sft & 0x1) - 1);
		uint16_t deg = sk[i]; // modified sk_s -> s
		for(j = 0; j < LWE_N; ++j){
		if(branch==0x0001){
