...
//// Step4 : Gen poly_e1 and poly_e2. //// 
	...
	shake256(tmp_e1, HE*3, Seed_e1, 32);
	
	while (hw < HE) {
		e1_random_idx = tmp_e1[count++]; 
		e1_random_idx <<= 8;	
		e1_random_idx ^= tmp_e1[count++];
		temp = (e1_random_idx & 0x02) - 1;
		e1_random_idx >>= 2;
		e1_random_idx &= (LWE_N - 1);  
		if (c1[e1_random_idx] == 0) {
			c1[e1_random_idx] = temp;
			hw++;
		}
		if (count >= (HE*3 - 2)) { 
	...
	while (hw < HE) {
		...
		e2_random_idx <<= 8;	
		e2_random_idx ^= tmp_e2[count++];
		temp = (e2_random_idx & 0x02) - 1;
		e2_random_idx >>= 2;
		e2_random_idx &= (LWE_N - 1);  
		if (c2[e2_random_idx] == 0) {
			c2[e2_random_idx] = temp<<(LOG_Q-LOG_P);
			hw++;
		}
		if (count >= (HE*3 - 2)) { 
