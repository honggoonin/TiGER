...
{
  int16_t r,m;
  r = coeff_freeze(x);//x % Modulus_Q;//1024;//coeff_freeze(x);
  printf("r : %i \t", r);
  r = r-(Modulus_Q/2);//128;//512;
  m = r >> 15;
  return (r + m) ^ m;
}
...
void poly_frommsg(poly *r, const uint8_t *msg)
{
  ...
  for(i=0;i<16;i++) // 
  {
    ...
    {
      mask = -((msg[i] >> j)&1);
      r->coeffs[8*i+j+  0] = (mask & (Modulus_Q/2));//<< _16_LOG_T;
      r->coeffs[8*i+j+128] = (mask & (Modulus_Q/2));//<< _16_LOG_T;
    }
  }
}
...
**************************************************/
void poly_tomsg(uint8_t *msg, poly *x){
	uint16_t t;
	for(size_t i=0; i<16; i++){msg[i] = 0;}
	for(size_t i=0;i<128;i++){
		printf("COE[%lu] : %u \t", i, x->coeffs[i]);		
		t  = flipabs(x->coeffs[i+  0]);//문제의 구간.... 뭔짓을 하는건데....
		printf("t[%lu] : %u \t", i, t);
