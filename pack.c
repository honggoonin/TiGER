...
#if LOG_P2 == 3
    size_t b_idx = 0; // 바이트 배열의 인덱스 초기화
    for (size_t i = 0; i < LWE_N; i += 8) {
        // 8개 계수를 3바이트로 압축
        uint32_t temp = 0;
        for (int j = 0; j < 8; ++j) {
            // 각 계수의 하위 3비트를 temp에 순차적으로 삽입
            temp |= ((data->coeffs[i + j] & 0x07) << (3 * (7 - j)));
        }
        // temp에서 바이트를 추출하여 bytes 배열에 저장
        bytes[b_idx++] = (temp >> 16) & 0xFF; // 상위 바이트
        bytes[b_idx++] = (temp >> 8) & 0xFF;  // 중간 바이트
        bytes[b_idx++] = temp & 0xFF;         // 하위 바이트
    }
#endif
}
 ...
 size_t d_idx = 0; // data->coeffs 배열의 인덱스
    for (size_t b_idx = 0; b_idx < CTPOLY2_BYTES; b_idx += 3) {
        ...
        if (b_idx + 1 < CTPOLY2_BYTES) {
            ...
            temp = (bytes[b_idx + 1] & 0x01) << 2; // 다음 계수의 첫 부분을 준비
        }
